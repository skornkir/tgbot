...
module.exports = function (bot) {
  bot.onText(/^\+заметка(?:\s+@(\S+)\s+(.+)|\s+(.+))?$/, async (msg, match) => {
    ...
    const newNote = match[2] || match[3];
    console.log(newNote);

    if (!newNote) {
      return bot.sendMessage(chatId, "❌ Укажи заметку после команды `+заметка`.", { reply_to_message_id: msg.message_id });
    }
   ...
   // const player  = await getPlayerDescription(targetTag);
    ...
    try { 
      ...
      await pool.query(
       `UPDATE clan_members SET notes = $1 WHERE lower(telegram_tag) = lower($2) and clan_id = $3`,
        [newNote, targetTag, clanId]
      );
      bot.sendMessage(
        chatId,
        `✅ Заметка для ${targetTag} обновлён: ${newNote}`,
